...
# Helper functions
# HF1. Split data into X, y, y_true
...
for (i in c(1:repeats)) {
  ...
  index <- str_pad(i, width = 2, pad = "0")
  train_data <- read.csv(glue("{path}/train_data_{index}.csv"))
  test_data <- read.csv(glue("{path}/test_data_{index}.csv"))
  ...
  if (overwrite) {
    index <- str_pad(i, width = 2, pad = "0")
    write.csv(s$spec_pars$Estimate, file = glue("models/experiment_{exp_id}/brms_sigma_{index}.csv"), row.names = F)
    # write.csv(re, file = glue("models/experiment_{exp_id}/brms_re_{index}.csv"), row.names = F)
    # write.csv(y_pred_train, file = glue("models/experiment_{exp_id}/brms_pred_train_{index}.csv"), row.names = F)
    write.csv(y_pred_test, file = glue("models/experiment_{exp_id}/brms_pred_test_{index}.csv"), row.names = F)
  }
  ...
  if (overwrite) {
    index <- str_pad(i, width = 2, pad = "0")
    write.csv(gp_model$get_cov_pars()[[1]], file = glue("models/experiment_{exp_id}/gpboost_scale_{index}.csv"), row.names = F)
    # write.csv(re, file = glue("models/experiment_{exp_id}/gpboost_re_{index}.csv"), row.names = F)
    # write.csv(y_pred_train, file = glue("models/experiment_{exp_id}/gpboost_pred_train_{index}.csv"), row.names = F)
    write.csv(y_pred_test, file = glue("models/experiment_{exp_id}/gpboost_pred_test_{index}.csv"), row.names = F)
  }
}
